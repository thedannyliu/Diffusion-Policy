...
class PushTEnv(gym.Env):
    ...
    def _handle_collision_new(self, arbiter, space, data):
        """Collision handler for pymunk 7.x API (on_collision callback)."""
        self.n_contact_points += len(arbiter.contact_point_set.points)
    ...
    def _setup(self):
        ...
        # Add collision handling (compatible with pymunk 7.x)
        # Use on_collision() for pymunk 7.x, add_default_collision_handler() for older versions
        self.n_contact_points = 0
        if hasattr(self.space, 'on_collision'):
            # pymunk 7.x API
            self.space.on_collision(post_solve=self._handle_collision_new)
