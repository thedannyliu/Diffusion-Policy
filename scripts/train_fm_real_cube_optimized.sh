#!/bin/bash
#SBATCH --job-name=fm_real_cube
#SBATCH --output=logs/fm_real_cube_%j.out
#SBATCH --error=logs/fm_real_cube_%j.err
...
# === Optimized FM Training for Real Robot (Cube) ===
# Paper-aligned settings for fair comparison with DDPM
# Usage: sbatch scripts/train_fm_real_cube_optimized.sh [seed] [steps]

SEED=${1:-42}
STEPS=${2:-8}

echo "=================================================="
echo "Flow Matching Real Robot Training - Cube"
echo "=================================================="
echo "Seed: $SEED"
...
srun -u python -m dpfm.train \
    ...
    optimizer.lr=1e-4 \
    exp_name=fm_real_cube_optimized \
    logging.project=dpfm_real_robot \
    logging.name="cube_fm_step${STEPS}_seed${SEED}"
