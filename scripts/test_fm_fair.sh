#!/bin/bash
#SBATCH --job-name=fm_fair_test
#SBATCH --output=logs/fm_fair_test_%j.out
#SBATCH --error=logs/fm_fair_test_%j.err
...
# Quick test: FM fair comparison version
...
echo "=== Testing FM Fair Comparison ==="
...
srun -u python dpfm/train.py \
    ...
    exp_name=test_fair \
    hydra.run.dir='./data/outputs/test/${now:%H.%M.%S}_fm_fair_test'
