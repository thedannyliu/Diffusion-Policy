"""
Training Workspace for Flow Matching UNet Hybrid Image Policy.

This workspace is aligned with TrainDiffusionUnetHybridWorkspace from the original paper.
Key alignment:
1. Uses FlowMatchingUnetHybridImagePolicy (FM version of DiffusionUnetHybridImagePolicy)
2. Same training loop, optimizer, scheduler, checkpointing
...
def compute_action_jerk(actions: torch.Tensor, dt: float = 0.1) -> Dict[str, float]:
    ...
    """
    if actions.dim() == 2:
        actions = actions.unsqueeze(0)  # Add batch dim
    ...
    # Compute statistics
    jerk_magnitude = torch.norm(jerk, dim=-1)  # [batch, T-3]
...
class TrainFlowMatchingUnetHybridImageWorkspace(BaseWorkspace):
    """
    Training workspace for Flow Matching Hybrid Image Policy.
    
    Aligned with TrainDiffusionUnetHybridWorkspace for fair comparison.
    Uses robomimic-style observation encoder with both image and low-dim observations.
    """
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        # Configure model (FlowMatchingUnetHybridImagePolicy)
        self.model: FlowMatchingUnetHybridImagePolicy = hydra.utils.instantiate(cfg.policy)

        # EMA model for evaluation (same as DDPM)
        self.ema_model: FlowMatchingUnetHybridImagePolicy = None
    ...
    def run(self):
        ...
        wandb.config.update({
            "output_dir": self.output_dir,
            "method": "flow_matching_hybrid",
            "num_inference_steps": cfg.policy.num_inference_steps,
            "policy_type": "FlowMatchingUnetHybridImagePolicy"
        })
        ...
        with JsonLogger(log_path) as json_logger:
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                if (self.epoch % cfg.training.rollout_every) == 0:
                    ...
                    # Log latency statistics from env_runner
                ...
                if (self.epoch % cfg.training.sample_every) == 0:
                    with torch.no_grad():
                        ...
                        # Compute action jerk for predicted actions
                        ...
                        # Also compute jerk for ground truth (for comparison)
...
def main(cfg):
    workspace = TrainFlowMatchingUnetHybridImageWorkspace(cfg)
    workspace.run()
